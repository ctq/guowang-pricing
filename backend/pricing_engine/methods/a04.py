...
def calculate(payload: CalculationInput, params: dict[str, Decimal]) -> CalculationResult:
    ...
    assign_ranks(rows)
    return CalculationResult(CODE, NAME, q(benchmark, 6) or benchmark, discount_rate(benchmark, payload.project.ceiling_price), len(active), len(active), target_result(rows, payload.project), rows, {"A1": q(a1, 6), "A2": q(a2, 6) if a2 is not None else None, "P_min": q(p_min, 6) if p_min is not None else None, "lower": q(lower, 6), "upper": q(upper, 6), "trim_high": high, "trim_low": low, "fallback": fallback, "benchmark_formula": "(A2 + P_min) / 2 or full average"})
